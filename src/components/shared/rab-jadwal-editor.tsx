...
/**
 * RAB & Jadwal row editors — shared between the per-SPH-item editor
 * (`penawaran/service-rab-jadwal-editor.tsx`) and the Konfigurasi RAB/Jadwal
 * template forms. Same `Rab`/`Jadwal` shape as `itemRabSchema`/
 * `itemJadwalSchema` (schemas/penawaran.ts) and `RabTemplate`/`JadwalTemplate`
 * (schemas/rab-templates.ts / schemas/jadwal-templates.ts), so a template can
 * be applied with a direct assign — no shape conversion needed.
 */
export type Rab = { personil: RabRow[]; langsung: RabRow[] };
export type Jadwal = { kegiatan: string[]; highlights: number[][]; bulan: number };

export function rabRowsTotal(rows: RabRow[]): number {
  return rows.reduce((s, r) => s + rabRowTotal(r), 0);
}

/* ---------- RAB ---------- */
export function RabEditor({ rab, onChange }: { rab: Rab; onChange: (next: Rab) => void }) {
  const jumlahA = rabRowsTotal(rab.personil);
  const jumlahB = rabRowsTotal(rab.langsung);
      ...
      <div className="space-y-2">
        {rows.map((row, i) => (
          ...
          >
            ...
            <div className="ml-auto pb-2 text-right whitespace-nowrap">
