...
describe("toLayanan", () => {
  it("resolves document type/authority/legal basis labels", () => {
    const result = toLayanan(service(), documentType, authority, legalBasis, 0, 0);
  ...
  it("falls back to '—' for jenisDokumen/kewenangan and null for dasarHukum when unresolved", () => {
    const result = toLayanan(service({ documentTypeId: null, authorityId: null, legalBasisId: null }), undefined, undefined, undefined, 0, 0);
  ...
  it("maps is_active true/false to status aktif/terarsip", () => {
    expect(toLayanan(service({ isActive: true }), documentType, authority, legalBasis, 0, 0).status).toBe("aktif");
    expect(toLayanan(service({ isActive: false }), documentType, authority, legalBasis, 0, 0).status).toBe("terarsip");
  });

  it("converts standardPrice to a number, or null when unset", () => {
    expect(toLayanan(service({ standardPrice: "50000" }), documentType, authority, legalBasis, 0, 0).hargaStandar).toBe(50000);
    expect(toLayanan(service({ standardPrice: null }), documentType, authority, legalBasis, 0, 0).hargaStandar).toBeNull();
  });

  it("passes through isRecurring", () => {
    expect(toLayanan(service({ isRecurring: true }), documentType, authority, legalBasis, 0, 0).isRecurring).toBe(true);
  });

  it("passes through the real dipakaiSPH/dipakaiProyek counts", () => {
    const result = toLayanan(service(), documentType, authority, legalBasis, 7, 4);
